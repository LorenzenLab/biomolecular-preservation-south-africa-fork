---
title: "DNA and collagen preservation modelling"
...
**Factors to consider:** 
  ...
  * noMicro_endoPropxTarget: This is the endogenous DNA proportion where microbe-like and mitochondrial-like regions have been masked.
  * latest_chrono_age: This is the estimated age of the specimen.
  * Site_short: The fossil sites, in shorthand.
  * Taxon_plots_common: The assigned taxon for plotting and modelling purposes.
...
```{r}
Data %>%
  ggplot(aes(x = latest_chrono_age, y = noMicro_endoPropxTarget)) +
  geom_jitter() + 
  labs(title = "Endogenous DNA content plotted against age")

Data %>%
  ggplot(aes(x = latest_chrono_age, y = log_endo)) +
  geom_jitter() + 
...
LM <- lm(log_endo ~ latest_chrono_age, data = Data)
...
Data$Site_short <- factor(Data$Site_short,
                         levels = c("NBC", "DK1", "EBC", "KRM", "BPA", "BNK1"))
LM_SS <- lm(log_endo ~ latest_chrono_age * Site_short, data = Data)
...
Data$material_group <- factor(Data$material_group,
                              levels = c("Tooth", "Bone"))
LM_MG <- lm(log_endo ~ latest_chrono_age * material_group, data = Data)
...
LM_SS_MG <- lm(log_endo ~ latest_chrono_age * Site_short * material_group, data = Data)
...
LM_TP <- lm(log_endo ~ latest_chrono_age * Taxon_plots_common, data = Data)
...
LM_SS_TP <- lm(log_endo ~ latest_chrono_age * Site_short * Taxon_plots_common, data = Data)
...
**The LM_SS model and ANOVA gave the following results**
  ...
  5. latest_chrono_age:Site_short significantly impacted endogenous content, F = 7.9459, Df = 5, p < 0.001
...
#### Load data in
```{r message=FALSE, warning=FALSE}
Data_CollagenAge <- read_csv("Table S3 - paleomix nuclear & collagen.csv")
...
```{r}
Data_CollagenAge %>%
  ggplot(aes(x = latest_chrono_age, y = Coll_Yield_perc)) +
  geom_jitter() + 
  labs(title = "Collagen Yield against age")

Data_CollagenAge %>%
  ggplot(aes(x = latest_chrono_age, y = log_Coll)) +
  geom_jitter() + 
...
GLM_collagen_age <- glm(Coll_Yield_adj ~ latest_chrono_age, data = Data_CollagenAge, 
...
#Data_CollagenAge$Site_short <- factor(Data_CollagenAge$Site_short,
#                         levels = c("NBC", "DK1", "EBC", "KRM", "BPA", "BNK1"))
#GLM_SS_collagen_age <- glm(Coll_Yield_adj ~ latest_chrono_age * Site_short, data = Data_CollagenAge,  
#                         family = Gamma(link = "inverse"))
...
Data_CollagenAge$material_group <- factor(Data_CollagenAge$material_group,
                              levels = c("Tooth", "Bone"))
GLM_MG_collagen_age <- glm(Coll_Yield_adj ~ latest_chrono_age * material_group, data = Data_CollagenAge, 
                          family = Gamma(link = "inverse"))
...
Data_CollagenAge$Taxon_plots_common <- factor(Data_CollagenAge$Taxon_plots_common,
                         levels = c("Cape buffalo", "Cape grysbok", "Common eland", "Grey rhebok", "Long-horned buffalo", "Reedbuck"))
GLM_TP_collagen_age <- glm(Coll_Yield_adj ~ latest_chrono_age * Taxon_plots_common, data = Data_CollagenAge, 
                             family = Gamma(link = "inverse"))
...
#Model considering the effect of age and its interaction with site on endogenous content
## Set levels, so that all sites are compared to NBC. See also options("contrasts")
Data_Collagen$Site_short <- factor(Data_Collagen$Site_short,
                         levels = c("NBC", "DK1", "EBC", "KRM", "BPA", "BNK1"))
LM_SS_collagen <- lm(log_endo ~ Coll_Yield_perc * Site_short, data = Data_Collagen)
...
#Model considering the effect of age and its interaction with site and material group on endogenous content
LM_SS_MG_collagen <- lm(log_endo ~ Coll_Yield_perc * Site_short * material_group, data = Data_Collagen)
...
#Model considering the effect of age and its interaction with site and taxon on endogenous content
LM_SS_TP_collagen <- lm(log_endo ~ Coll_Yield_perc * Site_short * Taxon_plots_common, data = Data_Collagen)
...
Overall, the best and most biologically-relevant model was likely the lm model that took into account site as an experimental variable. The independence, homoscedasticity, and normality looked correct according to the resid plots. 
  ...
  5. Coll_Yield_perc:Site_short significantly impacted endogenous DNA content: F = 2.7939, Df = 4, p < 0.05
...
```{r}
Data_Yes %>%
  ggplot(aes(x = latest_chrono_age, y = noMicro_meanLengthxTarget)) +
  geom_jitter() + 
  labs(title = "Mean read length plotted against age")

Data_Yes %>%
  ggplot(aes(x = latest_chrono_age, y = log_rl)) +
  geom_jitter() + 
...
LM_Yes <- lm(log_rl ~ latest_chrono_age, data = Data_Yes)
RVAideMemoire::plotresid(LM_Yes)
summary(LM_Yes)

#Model considering the effect of age and its interaction with site on endogenous content
## Set levels, so that all sites are compared to NBC. See also options("contrasts")
Data_Yes$Site_short <- factor(Data_Yes$Site_short,
                         levels = c("NBC", "DK1", "EBC", "KRM", "BPA", "BNK1"))
LM_SS_Yes <- lm(log_rl ~ latest_chrono_age * Site_short, data = Data_Yes)
...
Data_Yes$material_group <- factor(Data_Yes$material_group,
                              levels = c("Tooth", "Bone"))
LM_MG_Yes <- lm(log_rl ~ latest_chrono_age * material_group, data = Data_Yes)
...
Data_Yes$material_group <- factor(Data_Yes$material_group,
                              levels = c("Tooth", "Bone"))
LM_SS_MG_Yes <- lm(log_rl ~ latest_chrono_age * Site_short * material_group, data = Data_Yes)
...
Data_Yes$Taxon_plots_common <- factor(Data_Yes$Taxon_plots_common,
                         levels = c("Cape buffalo", "Cape grysbok", "Common eland", "Grey rhebok", "Long-horned buffalo", "Reedbuck"))
LM_TP_Yes <- lm(log_rl ~ latest_chrono_age * Taxon_plots_common, data = Data_Yes)
RVAideMemoire::plotresid(LM_TP_Yes)
summary(LM_TP_Yes)

#Model considering the effect of age and its interaction with site and taxon on endogenous content
LM_SS_TP_Yes <- lm(log_rl ~ latest_chrono_age * Site_short * Taxon_plots_common, data = Data_Yes)
...
**For this analysis the LM_SS model and ANOVA gave the following results**
  ...
  5. latest_chrono_age:Site_short significantly impacted mean read length, F = 4.4937, Df = 4, p < 0.01
...
#Model only considering the effect of age on read length
LM_Yes <- lm(log_rl ~ latest_chrono_age, data = Data_Yes_NBC)
RVAideMemoire::plotresid(LM_Yes)
summary(LM_Yes)
...
Data_Yes_NBC$material_group <- factor(Data_Yes_NBC$material_group,
                              levels = c("Tooth", "Bone"))
LM_MG_Yes <- lm(log_rl ~ latest_chrono_age * material_group, data = Data_Yes_NBC)
RVAideMemoire::plotresid(LM_MG_Yes)
summary(LM_MG_Yes)
...
LM_TP_Yes <- lm(log_rl ~ latest_chrono_age * Taxon_plots_common, data = Data_Yes_NBC)
RVAideMemoire::plotresid(LM_TP_Yes)
summary(LM_TP_Yes)

#Model considering the effect of age and its interaction with site and taxon on endogenous content
LM_MG_TP_Yes <- lm(log_rl ~ latest_chrono_age * material_group * Taxon_plots_common, data = Data_Yes_NBC)
